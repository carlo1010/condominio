...
@Component({
    selector: 'app-condomino-tipo-spesa-list',
    templateUrl: './list.component.html',
    styleUrls: ['./list.component.scss']

})

export class TipoSpesaListComponent implements OnInit, OnDestroy {
    ...
    public tipospesaListArray: any;
    public filterData: any = [];
    public filterDataId = [];


    constructor(public sandbox: TipoSpesaSandbox,
                private tipospesaApi: TipoSpesaApiClient,
                private route: Router,
                public configService: ConfigService,
                private toastr: ToastrManager) {
                  this.subscribeTipoSpesa();
    ...
    ngOnInit() {
        ...
        this.getTipoSpesaList();
        this.getTipoSpesaCount();
    ...
    getTipoSpesaList() {
        ...
        params.count = '';
        this.sandbox.tipospesaList(params);
    }
    ...
    getTipoSpesaCount() {
        ...
        params.count = 1;
        params.keyword = this.keyword;
        this.sandbox.tipospesaCount(params);
    }
     ...
     */
    editTipoSpesa(editValue) {
        this.tipospesaApi.setTipoSpesaEditValue(editValue);
        this.route.navigate(['condominio/tipo-spesa/edit', editValue.tipoSpesaId]);
    }

    addTipoSpesa() {
        this.tipospesaApi.setTipoSpesaEditValue('');
        this.route.navigate(['condominio/tipo-spesa/add']);
    }

    /**
     * A function 'deleteBrand' handle delete brandlist data
     * @param tipoSpesaId From manufacturerId
     */
    deleteTipoSpesa(tipoSpesaId, deletePop) {
        event.stopPropagation();
        this.sandbox.deletetipospesa({tipoSpesaId: tipoSpesaId});
    }

    // A function 'regSubscribeEvents'  call after getting delete response for pagination purpose
    regSubscriptionEvents() {
        this.sandbox.getTipoSpesaDelete$.subscribe(_delete => {
    ...
    selectAll() {
      for (let i = 0; i < this.tipospesaListArray.length; i++) {
        this.tipospesaListArray[i].selected = this.selectedAll;
      }
      this.filterDataList();
    ...
    checkIfAllSelected() {
      this.bulkFunction = true;
      this.selectedAll = this.tipospesaListArray.every(function(item: any) {
    ...
    filterDataList() {
      this.filterData = this.tipospesaListArray.filter(data => {
    ...
    subscribeTipoSpesa() {
      this.subscriptions.push(this.sandbox.getTipoSpesaList$.subscribe((data: any) => {
        this.tipospesaListArray = [];
        if (data && data.length > 0) {
          this.tipospesaListArray = data.map(list => {
    ...
    exportTipoSpesa() {
      ...
      this.sandbox.exportTipoSpesa(param);
    }
