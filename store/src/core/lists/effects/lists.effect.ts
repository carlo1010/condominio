...
export class ListsEffect {
  ...
  @Effect()
  getProductSearch$: Observable<Action> = this.actions$.pipe(
    ofType(actions.ActionTypes.GET_PRODUCTSEARCH_LIST),
    map((action: actions.GetProductSearchList) => action.payload),
    switchMap(state => {
      return this.authApi.getProductSearch(state).pipe(
        map(register => new actions.GetProductSearchListSuccess(register)),
        catchError(error => of(new actions.GetProductSearchListFail(error)))
      );
    })
  );
