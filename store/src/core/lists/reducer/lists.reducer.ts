...
// <------------ GET VENDOR PRODUCT LIST COUNT -----------> //
 ...
 // <------------ GET PRODUCT SEARCH LIST -----------> //
    ...
    case actions.ActionTypes.GET_PRODUCTSEARCH_LIST: {
      return Object.assign({}, state, {
    ...
    case actions.ActionTypes.GET_PRODUCTSEARCH_LIST_SUCCESS: {
      return Object.assign({}, state, {
        productsearchLoading: false,
        productsearchLoaded: true,
        productsearchFailed: false,
        getProductSearch: payload.data,
      });
    }

    case actions.ActionTypes.GET_PRODUCTSEARCH_LIST_FAIL: {
      return Object.assign({}, state, {
        productsearchLoading: false,
        productsearchLoaded: false,
    ...
    case actions.ActionTypes.CHANGE_WISHLIST_STATUS: {
      const product = state.products;
      const todayProduct = state.todayDeal;
      const featureProduct = state.featuredList;
      const widgetProduct = state.widgetProductList.data;
      if (payload) {
        if (payload.page === 'product') {
          if (payload.checked) {
            product.map(data => {
              if (data.productId === payload.details.productId) {
        ...
        } else if (payload.page === 'today deals') {
          if (payload.checked) {
            todayProduct.map(data => {
              if (data.productId === payload.details.productId) {
                data.wishListStatus = 1;
              }
            });
        ...
        } else if (payload.page === 'feature product') {
          if (payload.checked) {
            featureProduct.map(data => {
              if (data.productId === payload.details.productId) {
                data.wishListStatus = 1;
              }
            });
        ...
        }  else if (payload.page === 'widget') {
          if (payload.checked) {
             widgetProduct.map(data => {
              const widgets = data.items;
              widgets.map(items => {
                if (items.productId === payload.details.productId) {
                  items.wishListStatus = 1;
                }
              });
            });
          } else {
      ...
      return Object.assign({}, state, {
        products: product,
        todayDeal: todayProduct,
        featuredList: featureProduct
      });
    }
...
export const getProductSearch = (state: ListsState) => state.getProductSearch;
export const getProductSearchLoading = (state: ListsState) => state.getProductSearchLoading;
export const getProductSearchLoaded = (state: ListsState) => state.getProductSearchLoaded;
export const getProductSearchFailed = (state: ListsState) => state.getProductSearchFailed;
