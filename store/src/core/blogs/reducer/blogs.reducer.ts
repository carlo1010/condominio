...
export function reducer(
  ...
  switch (type) {
    ...
    case actions.ActionTypes.GET_BLOG_LIST_SUCCESS: {
      const tempList = payload.data.results.map(list => {
        const blog = new BlogListModel(list);
        return blog;
      });
      const temp = new BlogListModel(payload.data.featuredPost);
      return Object.assign({}, state, {
        blogList: tempList,
        featuredList: payload.data.featuredPost,
...
export const featuredList = (state: BlogsState) => state.featuredList;
