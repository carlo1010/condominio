...
export class AuthEffects {
  ...
  @Effect()
  verification$: Observable<Action> = this.actions$.pipe(
      ofType(actions.ActionTypes.DO_VERIFICATION),
      map((action: actions.Verification) => action.payload),
      switchMap(state => {
          return this.authApi.verification(state).pipe(
              map(register => new actions.VerificationSuccess(register)),
              catchError(error => of(new actions.VerificationFail(error)))
          );
      })
  );
  ...
  resetPassword$: Observable<Action> = this.actions$.pipe(
    ofType(actions.ActionTypes.DO_RESET_PASSWORD),
    map((action: actions.ResetPassword) => action.payload),
    switchMap(state => {
      return this.authApi.resetPassword(state).pipe(
        tap(val => {
          if (val && val.status) {
        ...
        map(verfication => new actions.ResetPasswordSuccess(verfication)),
        catchError(error => of(new actions.ResetPasswordFail(error)))
      );
    })
  );
}
