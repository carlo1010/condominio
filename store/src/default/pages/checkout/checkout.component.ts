...
export class CheckoutComponent implements OnInit, OnDestroy {
  ...
  billingAddressEventChange(event: any) {
    if (!event.checked) {
      ...
      this.checkoutForm.controls['billingZip'].setValidators([Validators.required]);
      this.checkoutForm.controls['billingZip'].updateValueAndValidity();

      this.checkoutForm.controls['billingAddress'].setValidators([Validators.required]);
  ...
  public accountCreated(param) {
    if (param.checked) {
      this.checkoutForm.controls['setPassword'].setValidators([Validators.required]);
  ...
  gstAvailable(event) {
    if (event.checked) {
      this.checkoutForm.controls['gst'].setValidators([Validators.required]);
