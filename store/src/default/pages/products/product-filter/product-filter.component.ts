...
export class ProductFilterComponent implements OnInit, OnChanges, OnDestroy {
  ...
  public priceClear = true;
  public showMoreControls: any;
  public priceMax: any;
  ...
  constructor(
    ...
    this.variantValue = this.route.snapshot.queryParamMap.get('variantValue') ? this.route.snapshot.queryParamMap.get('variantValue') : '';
    this.routeBrands = this.route.snapshot.queryParamMap.get('brand') ? this.route.snapshot.queryParamMap.get('brand') : '';
    this.filterPriceFrom = this.route.snapshot.queryParamMap.get('priceFrom') ? this.route.snapshot.queryParamMap.get('priceFrom') : 0;
    this.filterPriceTo = this.route.snapshot.queryParamMap.get('priceTo') ? this.route.snapshot.queryParamMap.get('priceTo') : '';
  }
    ...
    this.subscriptions.push(
      this.listSandbox.maxProductPrice$.subscribe(maximumPrice => {
        ...
        if (maximumPrice && this.oneTimeAssignValue === 1) {
          ...
          this.showMoreControls = this.rupees + this.priceFrom;
          this.priceMax = this.rupees + this.priceTo;
        }
      })
    );
  ...
  splitVariant() {
    if (this.variantValue !== '') {  //  public Selected=false;
      this.selectedVariant = {};
