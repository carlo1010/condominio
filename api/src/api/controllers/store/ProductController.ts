...
export class ProductController {
    ...
    public async productDetail(@Param('productslug') productslug: string, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        if (request.header('authorization')) {
            ...
            jwt.verify(originalEncryptedString, env.jwtSecret, { ignoreExpiration: true }, (err, decoded) => {
                if (err) {
                    throw err;
                }
    ...
    public async featureProductList(@QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @QueryParam('keyword') keyword: string, @QueryParam('sku') sku: string, @QueryParam('count') count: number | boolean, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        const promises = featureProduct.map(async (result: any) => {
            ...
            const review = await this.productRatingService.listByQueryBuilder(0, 0, 0, whereCondition1, 0, 0, 0, 0, true);
    ...
    public async productSearchList(@QueryParam('keyword') keyword: string, @Res() response: any, @Req() request: any): Promise<any> {
        const select = ['productId', 'sku', 'name', 'quantity', 'price', 'isFeatured', 'todayDeals', 'productSlug', 'isActive'];
        const search = [];
        ...
        const productSearchList = await this.productService.list(0, 0, select, [], whereCondition, search, 0, 0);
        const productList = productSearchList.map(async (value: any) => {
            ...
            if (productToCategory) {
                const category = await this.categoryService.findOne({
                    select: ['categoryId', 'name', 'isActive'],
                    where: {
