...
export class CommonListController {
    constructor(
        ...
        private vendorProductService: VendorProductService, private vendorService: VendorService,
    ...
    public async customProductList(
        ...
        return new Promise(async () => {
            ...
            const selects = ['Product.productId as productId',
                ...
                '(SELECT sku.price FROM sku JOIN product_varient_option as pvo ON pvo.sku_id=sku.id WHERE pvo.product_id = `Product`.`product_id`  ORDER BY sku.price ASC LIMIT 1) as varientPrice',
                '(SELECT sku.sku_name FROM sku JOIN product_varient_option as pvo ON pvo.sku_id=sku.id WHERE pvo.product_id = `Product`.`product_id`  ORDER BY sku.price ASC LIMIT 1) as varientSkuName',
                '(SELECT sku.id FROM sku JOIN product_varient_option as pvo ON pvo.sku_id=sku.id WHERE pvo.product_id = `Product`.`product_id`  ORDER BY sku.price ASC LIMIT 1) as varientSkuId',
                '(SELECT tax.tax_percentage FROM tax WHERE tax.tax_id = `Product`.`tax_value` AND `Product`.`tax_type` = 2 LIMIT 1) as taxPercentageValue',
                ];
                const whereCondition = [];
            const relations = [];
            const groupBy = [];
            ...
            if (params.price) {
                sort.push({
                    name: '(IF (is_simplified = 1, (CASE WHEN special != NULL THEN special WHEN discount != NULL THEN discount WHEN taxPercentageValue != NULL THEN (taxPercentageValue/100 * `Product`.`price`) + `Product`.`price`  ELSE `Product`.`price` END), varientPrice))',
                    order: params.price,
                });
            } else {
                sort.push({
            ...
            const promises = productList.map(async (result: any) => {
                ...
                if (result.isSimplified === 1) {
                    ...
                    // }
                    const sku = await this.skuService.findOne({ id: result.skuId });
                    temp.price = result.varientPrice ? result.varientPrice : sku.price;
                    temp.skuName = result.varientSkuName ? result.varientSkuName : sku.skuName;
                    skuId = result.varientSkuId ? result.varientSkuId : sku.id;
                }
                if (skuId) {
                    const nowDate = new Date();
                    const todaydate = nowDate.getFullYear() + '-' + (nowDate.getMonth() + 1) + '-' + nowDate.getDate();
                ...
                if (request.header('authorization')) {
                    ...
                    const bytes  = Crypto.AES.decrypt(encryptString, env.cryptoSecret);
                    const originalEncryptedString = bytes.toString(Crypto.enc.Utf8);
                    const userId = jwt.verify(originalEncryptedString, env.jwtSecret, { ignoreExpiration: true });                    const userUniqueId: any = Object.keys(userId).map((key: any) => {
                        return [(key), userId[key]];
                    });
                    const wishStatus = await this.customerWishlistService.findOne({
                        where: {
    ...
    public async widgetList(@QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @QueryParam('count') count: number | boolean, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        const promise = widgetList.map(async (result: any) => {
            ...
            const promises = productList.map(async (resultData: any) => {
                ...
                const review = await this.productRatingService.listByQueryBuilder(0, 0, 0, whereConditions, 0, 0, 0, 0, true);
                tempVal.ratingCount = rating ? rating : 0;
                tempVal.reviewCount = review ? review : 'null';
                tempVal.skuName = '';
                let skuValue = undefined;
                let skuId = undefined;
