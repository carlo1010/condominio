...
export class DeliveryLocationController {
    ...
    constructor(private deliveryLocationService: DeliveryLocationService,
                private deliveryLocationToLocationService: DeliveryLocationToLocationService,
    ...
    public async addDeliveryLocation(@Body({ validate: true }) deliveryLocationParam: CreateDeliveryLocationRequest, @Req() request: any, @Res() response: any): Promise<any> {
        const exitsDeivery = await this.deliveryLocationService.findOne({ where: {zipCode: deliveryLocationParam.zipCode, vendorId: request.user.vendorId}});
        if (exitsDeivery) {
    ...
    public async deleteDeliveryLocation(@Param('deliveryLocationId') deliveryLocationId: number, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        const mappedLocation = await this.deliveryPersonToLocationService.findOne({
            where: { deliveryLocationId: deliveryLocation },
        });
        if (mappedLocation) {
            const errorResponse: any = {
                status: 0,
                message: 'unable to delete mapped location',
            };
            return response.status(400).send(errorResponse);
        }
    ...
    public async updateDeliveryLocation(@Body({ validate: true }) updateParam: CreateDeliveryLocationRequest,
        ...
        const exitsDeivery = await this.deliveryLocationService.findOne({ where: {zipCode: updateParam.zipCode, vendorId: request.user.vendorId , deliveryLocationId: Not(deliveryLocationId) }});
        if (exitsDeivery) {
            const errorResponse: any = {
                status: 0,
                message: 'This name already exits.',
    ...
    public async importDeliveryLocation(@UploadedFile('deliveryLocationData') file: any, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        for (const data of subjectData) {
            ...
            const exitsDeivery = await this.deliveryLocationService.findOne({ where: {zipCode: data.Pincode}});
            if (exitsDeivery) {
                const errorResponse: any = {
                    status: 0,
                    message: 'This name already exits.',
