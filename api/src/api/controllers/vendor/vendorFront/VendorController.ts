...
export class VendorController {
    ...
    public async login(@Body({ validate: true }) loginParam: VendorLogin, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        if (await Customer.comparePassword(resultData, loginParam.password)) {
            // create a token
            const token = jwt.sign({ id: findVendor.vendorId }, env.jwtSecret);
    ...
    public async uploadCustomerDocument(@BodyParam('customerData') file: string, @BodyParam('title') title: string, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        const availableTypes = env.availAllowTypes.split(',');
                if (!availableTypes.includes(fileType)) {
                    const errorTypeResponse: any = {
                        status: 0,
                        message: 'Only ' + env.availAllowTypes + ' types are allowed',
                    };
                    return response.status(400).send(errorTypeResponse);
                }
        let uploadPath;
    ...
    public async forgetPasswordLink(@QueryParam('email') emailId: string, @Res() response: any, @Req() request: any): Promise<any> {
        ...
        const logo = await this.settingService.findOne();
        const redirectUrl = env.vendorForgetPasswordLink;
    ...
    /**
     * @api {get} /api/vendor/forgot-password-key-check/:key Forgot Password Key check API
     * @apiGroup   Vendor
     * @apiSuccessExample {json} Success
     * HTTP/1.1 200 OK
     * {
    ...
    // Logout API
    /**
     * @api {get} /api/vendor/logout Log Out API
     * @apiGroup Vendor
     * @apiHeader {String} Authorization
     * @apiSuccessExample {json} Success
     * HTTP/1.1 200 OK
