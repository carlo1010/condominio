...
export class DeliveryLocationController {
    ...
    public async addDeliveryLocation(@Body({ validate: true }) deliveryLocationParam: CreateDeliveryLocationRequest, @Req() request: any, @Res() response: any): Promise<any> {
        const existDelivery = await this.deliveryLocationService.findOne({ where: {zipCode: deliveryLocationParam.zipCode}});
        if (existDelivery) {
            const errorResponse: any = {
                status: 0,
                message: 'you already added this zipcode.',
            };
    ...
    public async updateDeliveryLocation(@Body({ validate: true }) updateParam: CreateDeliveryLocationRequest,
        ...
        const existDelivery = await this.deliveryLocationService.findOne({ where: {zipCode: updateParam.zipCode, deliveryLocationId: Not(deliveryLocation.deliveryLocationId) }});
        if (existDelivery) {
            const errorResponse: any = {
                status: 0,
                message: 'you already added this zipcode.',
