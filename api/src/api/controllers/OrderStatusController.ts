...
export class OrderStatusController {
    ...
    public async createOrderStatus(@Body({ validate: true }) orderStatusParam: CreateOrderStatus, @Res() response: any): Promise<any> {
        const existOrder = await this.orderStatusService.findOne({ where: {name: orderStatusParam.name}});
        if (existOrder) {
            const errorResponse: any = {
                status: 0,
                message: 'you already added this name.',
            };
    ...
    public async updateOrderStatus(@Body({ validate: true }) orderStatusParams: CreateOrderStatus, @Param('id') id: number, @Res() response: any, @Req() request: any): Promise<any> {
        ...
        const existOrder = await this.orderStatusService.findOne({ where: {name: orderStatusParams.name, orderStatusId: Not(orderStatus.orderStatusId) }});
        if (existOrder) {
            const errorResponse: any = {
                status: 0,
                message: 'you already added this name.',
