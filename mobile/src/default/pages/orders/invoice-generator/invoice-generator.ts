...
export class InvoiceGenerator {
  ...
  downloadInvoiceDetail(details) {
    ...
    const item1 = this.invoiceDetail.orderedItems.map((item, index) => {
      return [index + 1, item.productName, item.quantity, item.price];
    });
  ...
  async showLoader() {
    this.loading = true;
    const currentLoading = await this.loadingController.create({
      message: 'Downloading...',
      backdropDismiss: true
    });
    return await currentLoading.present();
  }
  async hideLoader() {
    if (this.loading && this.loadingController.getTop() !== undefined) {
      setTimeout(async () => {
  ...
  generatePdf() {
          ...
          columns: [
            ...
            },
            {
              ...
              stack: [
                ...
                // {style: 'h2', text: 'dummy on line 68'},
                // {style: 'h2', text: 'dummy on line 69'},
                // {style: 'h2', text: 'dummy on line 67'},
              ]
            }
          ]
        },
        ...
        '\n',
        {
          columns: [
            ...
            },
            {
              width: '20%',
              stack: [
                { style: 'invoice_d', text: this.invoiceDetail.orderNumber },
                { style: 'h2', text: '' },
                {
                  style: 'invoice',
                  text: this.datePipe.transform(
                    this.invoiceDetail.orderedDate,
                    'dd/MM/yyyy'
                  )
                },
                { style: 'h2', text: '' },
          ...
          columns: [
            {
              width: '80%',
              alignment: 'right',
              stack: [{ style: 'h2', text: 'Total' }]
            },
            {
              width: '20%',
              alignment: 'right',
              stack: [{ style: 'h2', text: this.invoiceDetail.totalPrice }]
            }
          ]
        }
      ],
    ...
    this.pdf = pdfMake;
    // this.pdf.createPdf(this.docDefinition).download('invoice' + this.invoiceDetail.orderNumber);
    this.pdfObj = this.pdf.createPdf(this.docDefinition);
    this.downloadPdf();
  }
  ...
  async downloadPdf() {
    if (this.platform.is('cordova')) {
      this.pdfObj.getBuffer(buffer => {
        ...
        const fileName = 'invoice -' + this.invoiceDetail.orderNumber + '.pdf';
        this.file
